...
import {generateContextualAnimations} from "@/services/nvidia";
import {z} from "zod";
...
export async function POST(request: Request) {
  try {
    ...
    if (!apiKey) {
      return Response.json({message: "NVIDIA key is missing."}, {status: 400});
    }
    ...
    console.error("NVIDIA animation error:", error);
